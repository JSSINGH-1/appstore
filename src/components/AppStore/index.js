...
class AppStore extends Component {
  state = {selectedTabId: '', filterList: appsList}

  buttonEl = tabId => {
    console.log(tabId)
    this.setState({selectedTabId: tabId})
    const filterData = appsList.filter(eachData =>
      eachData.category.includes(tabId),
    )
    this.setState({filterList: filterData})
  }

  searchInput = event => {
    const {filterList} = this.state
    console.log(filterList)
    const updateData = filterList.filter(eachData =>
      eachData.appName.includes(event.target.value),
    )
    console.log(updateData)

    this.setState({filterList: updateData})
  }

  render() {
    const {filterList, selectedTabId} = this.state
    console.log(selectedTabId)
    return (
      <div className="bgContainer">
        <h1 className="heding">App Store</h1>
        ...
        <ul className="TablelistContainer">
        ...
        <ul className="app-container">
          {filterList.map(each => (
            <AppListItem key={each.appId} eachData={each} />
          ))}
        </ul>
      </div>
    )
